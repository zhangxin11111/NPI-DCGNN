...
def parse_args():
    ...
    parser.add_argument('--model_code', default=3, type=int, help='model code') # 1 2 3
...
if __name__ == "__main__":
    ...
    result_path = f'../result/{args.dataset}_{args.model_code}'
    ...
    test_dataset = Subgraph(dataset_test_path)
    # 打乱数据集
    ...
    #创建模型
    print('===',train_dataset[0]['source'].x.shape[1])
    ...
    Accuracy, Precision, Sensitivity, Specificity, MCC,AUC,pred_prob = Accuracy_Precision_Sensitivity_Specificity_MCC(model, test_loader, device,log_path)
    output = 'testing dataset, Accuracy: {:.5f}, Precision: {:.5f}, Sensitivity: {:.5f}, Specificity: {:.5f}, MCC: {:.5f}, AUC: {:.5f}'.format(Accuracy, Precision, Sensitivity, Specificity, MCC,AUC)
    print(output)
    write_log(log_path,output + '\n')
    if not osp.exists(f'{result_path}/pred_prob'):
        os.makedirs(f'{result_path}/pred_prob')
    with open(f'{result_path}/pred_prob/model.txt','a') as f:
        #f.write(f'RNA\tProtein\tOrigin Lable\tPredict Prob\n')
        for i in pred_prob:
            f.write(f'{i[0]}\t{i[1]}\t{i[2]}\t{i[3]}\n')

    output = f'{Accuracy},{Precision},{Sensitivity},{Specificity},{MCC},{AUC}'
    write_log(log_path, output + '\n')
    write_result( f'{result_path}/performance.txt',output)
    # 完毕
    end_time = time.time()
